...
contract supply_Chain {
    ...
    track_product[][] public tracks_array;
    ...
    function newProduct(uint own_id, string name ,uint p_cost ,string p_specs ,string p_review) returns (uint) {
        if(keccak256(participants[own_id]._userType) == keccak256("Manufacturer")) {
            ...
            tracks_array[product_id].push(track_product
            ({
                _product_id : product_id,
                _owner_id : own_id,
                _product_owner : participants[own_id]._userAddress,
                _timeStamp : now
            }));
        ...
        if(keccak256(p1._userType) == keccak256("Manufacturer") && keccak256(p2._userType)==keccak256("Supplier")){
           /*trk._product_id = prod_id;
            ...
            //tracks[track_id]._product_owner = msg.sender;
             ...
             tracks_array[prod_id].push(track_product({
                _product_id : prod_id,
                _owner_id : user2_id,
                _product_owner : participants[user2_id]._userAddress,
                _timeStamp : now
            }));
